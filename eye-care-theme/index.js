...
const parseRgbColor = (raw) => {
    ...
    if (mFunc) {
        ...
        if ([r, g, b].some((x) => Number.isNaN(x) || x < 0 || x > 255))
            return null;
        return { r, g, b };
    }
...
const rgbToCSSVarValue = (rgb) => `${rgb.r} ${rgb.g} ${rgb.b}`;
const opacityToPercent = (opacity) => `${Math.round(clamp(opacity, 0, 0.5) * 100)}%`;
...
const shouldEnableByTime = (nowMinutes, startMinutes, endMinutes) => {
...
const getTintRGB = (cfg) => {
    if (cfg.preset !== "custom")
        return PRESET_RGB[cfg.preset];
...
class EyeCareThemePlugin extends Plugin {
    constructor() {
        super(...arguments);
        this.config = { ...DEFAULT_CONFIG, smartTime: { ...DEFAULT_CONFIG.smartTime } };
        this.saveTimer = null;
        ...
        this.rootClassName = "petal-eye-care-enabled";
        this.baseVars = {
    ...
    onunload() {
        ...
        if (this.smartTimer)
            window.clearInterval(this.smartTimer);
        this.saveTimer = null;
        this.smartTimer = null;
        this.lastSmartApplied = null;
        this.enabledInput = null;
    ...
    createPresetSelect() {
        ...
        select.addEventListener("change", () => {
            ...
            if (!["yellow", "green", "gray", "custom"].includes(next))
                return;
    ...
    syncSettingUI() {
        if (this.enabledInput)
            this.enabledInput.checked = this.config.enabled;
        if (this.presetSelect)
            this.presetSelect.value = this.config.preset;
        if (this.customColorInput)
            this.customColorInput.value = this.config.customColor;
        if (this.opacityRange)
            this.opacityRange.value = String(Math.round(this.config.opacity * 100));
        if (this.smartTimeInput)
            this.smartTimeInput.checked = this.config.smartTime.enabled;
        if (this.smartStartInput)
            this.smartStartInput.value = this.config.smartTime.start;
        if (this.smartEndInput)
            this.smartEndInput.value = this.config.smartTime.end;
    }
    setupSmartTimer() {
        if (this.smartTimer)
            window.clearInterval(this.smartTimer);
        ...
        if (!this.config.smartTime.enabled)
            return;
        this.tickSmartTime();
        this.smartTimer = window.setInterval(() => this.tickSmartTime(), 30000);
    }
    tickSmartTime() {
        if (!this.config.smartTime.enabled)
            return;
        ...
        if (this.lastSmartApplied === shouldEnable)
            return;
        this.lastSmartApplied = shouldEnable;
        this.config.enabled = shouldEnable;
        if (this.enabledInput)
            this.enabledInput.checked = shouldEnable;
        this.applyTheme(true);
        this.persistConfigDebounced();
    }
    applyTheme(forceRebase = false) {
...
exports.default = EyeCareThemePlugin;
